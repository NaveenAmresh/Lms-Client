...
export class IssuedListComponent implements OnInit {
  print: ExportAsConfig = {type: 'pdf', elementId: 'table'}; text: ExportAsConfig = {type: 'doc', elementId: 'table'}; f3: any = '';
  csv: ExportAsConfig = {type: 'csv', elementId: 'table'}; excel: ExportAsConfig = {type: 'xls', elementId: 'table'}; f2: any = '';
  displayedColumns: string[] = ['index', 'user', 'mobile', 'email', 'issued_by', 'issued_on', 'issued_limit', 'status', 'action'];
  dataSource: MatTableDataSource<IssuedRow> = new MatTableDataSource(); private pagesize: number[]; loading: boolean = true;
  ...
  constructor(private datePipe: DatePipe, public dialog: MatDialog, public _get: ListIssueService,
              public exportAsService: ExportAsService) {} ngOnInit() { this.searchBooks(1); }
